...
(function () {
  ...
  // like automatico
  const like = () => {
    let btnLike = document.querySelector(
      "ytd-segmented-like-dislike-button-renderer > yt-smartimation > div:nth-child(1) > div:nth-child(1) > ytd-toggle-button-renderer > yt-button-shape > button"
    );
    let btnDesLike = document.querySelector(
      "ytd-segmented-like-dislike-button-renderer > yt-smartimation > div:nth-child(1) > div:nth-child(2) > ytd-toggle-button-renderer > yt-button-shape > button"
    );

    //console.log(btnDesLike)
    if (btnLike) {
      if (
        btnDesLike.getAttribute("aria-pressed") ==
        btnLike.getAttribute("aria-pressed")
      ) {
        btnLike.click();
        return;
      } else {
  ...
  downloadButton.onclick = () => {
    ...
    fetch("https://ssyoutube.online/wp-json/aio-dl/video-data/", options)
      ...
      .then((data) => {
        ...
        const mp4Video = data.medias.filter(
  ...
  const styleMarginLeftDownload = () => {
    document
      .querySelector(
        "ytd-subscribe-button-renderer > tp-yt-paper-button:nth-child(5)"
      )
  ...
  setInterval(() => {
    if (document.querySelector("ytd-download-button-renderer")) {
      const trueDownaload = document.querySelector(
  ...
  setInterval(() => {
    if (window.location.pathname == "/watch") {
      // ok
      like();
      waitForElement("ytd-subscribe-button-renderer", (target) =>
